...
df_state_winners %>% 
  {ggplot(., aes(x = reorder(state, p_candidate1), y = p_candidate1)) +
      geom_col(aes(fill = winner)) +
      scale_y_continuous(labels = scales::percent) +
      coord_flip() +
      xlab('') +
      ylab('Biden win probability') +
      ggtitle('Biden win probabilities',
              subtitle = paste0('Assuming DEM polling bias of ', candidate1_bias))}

df_state_winners %>% 
  dplyr::select(state, n, mean, sd, candidate1_bias, p_candidate1, t_stat, winner) %>% 
  DT::datatable()
```
