...
predict_state_winners <- function(df_polls,
  ...
  df_state_winners <- df_polls %>% 
    ...
    mutate(n = length(data$pct_diff),
           ...
           t_test = list(t.test(data$pct_diff - candidate1_bias, alternative = 'less')),
           #tdist_fitparams = list(MASS::fitdistr(data$pct_diff, densfun = 't', start = list(m=mean,s=sd, df=n), lower=c(-1, 0.001,1))$estimate)
    ) %>% 
    mutate(p_candidate1 = t_test$p.value,
           t_stat = t_test$statistic) %>% 
    # mutate(tdist_m = tdist_fitparams[['m']],
    #        tdist_s = tdist_fitparams[['s']],
    #        tdist_df = tdist_fitparams[['df']]) %>% 
...
```{r}
# states_selected <- c('Pennsylvania', 'Florida', 'Arizona', 'Wisconsin', 'North Carolina', 
#                                         'Michigan', 'Minnesota', 'Georgia', 'Nevada', 'Texas', 'Ohio', 
#                                         'Colorado', 'New Hampshire', 'Nebraska CD-2')

states_to_exclude <- c('Illinois', 'New York', 'Maine CD-1', 'West Virginia', 'Vermont',
                                                'South Dakota', 'Mississippi', 'Maryland', 'Massachusetts', 'Kentucky',
                                                'Oklahoma', 'Nebraska CD-2', 'District of Columbia', 'Nebraska CD-1')
states_selected <- all_states[!all_states %in% states_to_exclude]
